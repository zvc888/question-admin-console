<template>
  <div class="app-container">
    ...
    <data-grid url="/sysmgr/notice/list" dataName="listQuery" ref="dataList" @dataRest="onDataRest" >
      <template slot="form">
        <el-form-item label="标题">
          <el-input v-model="listQuery.title" placeholder="标题" size="small" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        ...
        <el-form-item label="状态">
          <el-select v-model="isRead" placeholder="全部"  @keyup.enter.native="handleFilter" size="small" class="filter-item status-min">
      ...
      <template slot="body">
        ...
        <el-table-column align="center" prop="content" label="內容" ></el-table-column>
        <el-table-column align="center" prop="status" label="状态" >
          <template slot-scope="scope">
            <el-tag :type="scope.row.status === '1'?'发布':'草稿'  ">{{ scope.row.status === '1'?'发布':'草稿'}}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="modifiedTime" label="修改时间" >
          <template slot-scope="scope">
  ...
  watch: {
    modifyVisible(val) {
      if(!val){
        ...
        this.scheduleForm.beginTime=null;
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    dataValid() {
      ...
      if(this.visitTimeRange){
        this.listQuery.beginTime= this.visitTimeRange[0];
        this.listQuery.endTime= this.visitTimeRange[1];
      }
    },
    modify(user){
